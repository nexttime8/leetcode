...
8. 一些数组知识
   1. 想要在原数组上修改数据，不会实现
9. 终于做出来了，也是用双指针，达到了所要求的时间复杂度![比较两个数组中的元素并顺序合并+处理nums2中的剩余元素](image-1.png)
   - 正因为会处理 nums2 中的剩余元素，也就不需要单独考虑 m=0 了
10. 基于上面方法，进一步优化![再优化性能](image.png)
...
1. 不会做
2. 只会用和上面同样的方法![时间复杂度不符合要求](image-3.png)

# 121 买股票的最佳时机

1. Math.max()方法要将一个个数传进去，所以数组要用扩展运算符
2. 我的方法一，将每一个元素与后面的所有元素组成的数组进行比较，得出一个最大值，但是超出时间限制![超出时间限制](image-2.png)
3. 我的方法二，将数组的数值和索引存入对象，并对数组进行排序，好像难以实现？
4. 我的方法三，直接找出元素的最大值，如果它的索引不为 0，就找出下标比他小的数组最小值；[3,3,5,0,0,3,1,4]这个例子推翻了我的方法。![存在问题](image-4.png)

# 169 多数元素 +

1. map 设置元素值总要用 set 方法
2. 完成了，性能不错![思路清晰一遍过](image-5.png)

# 217 存在重复元素

1. 和 169 实现方法一样，性能很差，主要是空间复杂度过高![Map空间复杂度高](image-6.png)
2. 使用 Object，多次提交终于做对，性能也很差，和 Map 差的不行上下![Alt text](image-7.png)
