## 两个电脑的路径不同

1. mac：/Volumes/世界如此美妙/job-orientation/leetcode
2. windows：e:/job-orientation/leetcode
3. ctrl+shift+c 一键召唤cmd

## 1 两数之和 + +

1. (实现方法一)双层循环暴力解![双层循环暴力解性能](images/image-1.png)
2. (实现方法二)用 Map 实现,学过 Map 且掌握了,但是不会用来解题![for...in+Map实现性能](images/image-2.png)
3. (实现方法三)用 Object 实现![for...in+Object实现性能](images/image-3.png)
4. 无论是 Object 实现,还是 Map 实现,看似是求两数之和
   1. 实则要有两数之差的理念
   2. 将值作为键,索引作为值存入 Map 对象和 Object 对象
5. 再次实现两数之和，发现 for...in 遍历和 for(let;<;++)遍历两种，时间和空间复杂度也不尽相同![for...let+Map实现性能](images/image-24.png)![for...let+Object实现性能](images/image-25.png)
   - for...let 比 for...in 空间性能更好
6. 直接以字面量的方式创建对象{}，点运算符不会有属性和方法的自动提示，而 new Object()有自动提示；
7. 二次尝试才发现，Map 的实现和 Object 的实现不同
   1. Map 因为键的唯一性，所以只在循环的特定时候将元素和索引加到 Map 中，set 方法，会覆盖前面已经存在的键的值
   2. Object 没有唯一性要求，所以每一次循环都将元素和索引加到 Object 中
   3. 相同点就是，两者都需要先判断是否存在当前 nums[i]与 target 的差值，先处理存在的情况；只有在不存在的时候，才会将元素存入 Map 或 Object
   4. Object 没有特定的方法用于判断元素是否存在，而是直接通过[]获取值并判断是否为 undefined

## 704 二分查找 + +

1. tips
   1. while 循环条件有没有=
   2. if 条件是 middle-1 还是 middle
2. 左闭右开?左闭右闭?两种情况写代码
3. int 型相加越界?
4. (实现方法一)二分查找左闭右闭实现![二分查找左闭右闭实现性能比较](image.png)
5. (实现方法二)二分查找左闭右开实现![二分查找左闭右开实现性能比较](images/image-4.png),比较发现左闭右开实现效果更好
6. 两次调整
   1. 第一次调整:js 中输出小数!不会自动向下取整;加上 parseInt()强制类型转换
   2. 第二次调整:考虑 target 不存在的情况[没认真看题目:如果目标值存在返回下标，否则返回 -1]
7. 二次尝试发现，根据 nums[middle]与 target 比较的 left 和 right 的取值，要弄清楚
8. 看 top voted solution，一种不同的取 middle 方式，`let mid = lo + Math.floor((hi - lo + 1) / 2)`也是`let mid = left + ((right - left) >> 1);`；且最终的目标是 lo，最终的索引包存在 lo 里面；循环条件是 `lo<ro`，性能很不错![二分查找top voted solution中lo保存最终索引](images/image-26.png)

## 4 寻找两个正序数组的中位数 + +

1. 一些知识点的巩固
   1. 数组解构再合并,新数组长度是两个原数组长度之和
   2. sort 方法,是生成新数组;sort 方法的时间复杂度
2. 一次调整![第一次尝试出错](images/image-5.png)
   1. 第一次调整:取数组中的元素的索引只能是整数,加上 parseInt()强制类型转换,或者 Math.floor()
3. (实现方法一)时间复杂度不符合要求(m+n)log(m+n).数组结构合并,sort 方法对数组进行排序,分两类情况返回中位数![解构加排序方法实现性能比较](images/image-6.png)
   - 自己也意识到了,没有利用到原本的两个数组是有序的,这个条件
4. (实现方法二)88 题,利用归并排序实现,合并两个有序数组,时间复杂度 `O(n+m)`
5. (实现方法三)二分法,时间复杂度 `O(log(min(m,n)))`,不需要完全合并两个数组![二分法](images/image-7.png)
   1. 看到时间复杂度要求 log,就要想到二分
   2. 无论是一个数组还是两个数组,都可以利用二分查找的思想;把数组分成两个部分,始终保持一个规则(左右个数相等或者左少一或者左多一),此外保证左边所有元素<=右边所有元素.(用以下任意一种逻辑都可以实现)
      1. 保证第一个数组的左边的最大元素<=第二个数组的右边的最小元素
      2. 保证第二个数组的左边的最大元素<=第一个数组的右边的最小元素
   3. 针对总长度分情况
      1. 总长度为奇数,如果左边多一个元素,则左边最大的是中位数
      2. 总长度为偶数,则左边最大和右边最小的均值为中位数
   4. 用下取整,可以不用对总长度是奇数还是偶数进行讨论?
      1. Math.floor((m+n+1)/2)
   5. 要保证较短数组的左右两侧都有元素,因此以较短的数组确定分割线位置;若无法保证有元素,需要进行特殊讨论
6. 最后自己写出来了,但是还是那个边界条件不理解![自己写的](images/image-8.png)
   1. 几种边界条件处理:1）数组为空，所以没有元素可以访问；2）数组的索引越界，也就是你想访问的元素的位置超出了数组的长度
   2. 以这个为例`let maxLeft1 = mid1 === 0 ? Number.MIN_SAFE_INTEGER : nums1[mid1 - 1]`
   3. 当 mid1=0,也就是左边没有元素的时候,将 maxLeft1 赋值为 JavaScript 能表示的最小安全整数,这样保证了 maxLeft1 一定小于 minRight2,从而不会影响中位数的结果(在判断是否符合二分条件的时候有用)
7. 二次尝试，还是不清楚，存在较多问题
   1. right 是将 n1 作为初始值
   2. 四个变量是通过 mid1&mid2 与 0 和 n1&n2 比较
   3. 赋极端值有两个意义
      1. 判断是否找到最终分割点，排除边界
      2. 取中位数值的时候，排除边界
   4. 数组总长为偶数，不需要 Math.floor
   5. while 循环里面没找到时，反过来用 mid1 来更新 left 和 right 值
      1. 如果 nums1 左边的最大值比 nums2 的右边最小值大，将 left 赋值为 mid1-1
      2. 同理
   6. 不要忘记边界处理，也就是最外层 return -1

## 26 删除有序数组中的重复项 + + ✔

1. for 循环加 splice 删除元素![次优解O(n^2)时间长度天差地别](images/image-10.png)
   1. 看似只遍历一次数组,但是 splice 方法的时间复杂度是`O(n)`,而且嵌套在 for 循环内
2. 直接原地赋值![更优解O(n)](images/image-9.png)
   1. for 循环的循环变量是一个指针,变量 i 是一个指针,使用双指针实现(双指针法特别适合于遍历数组时进行比较的情况)
   2. 一个快指针一个慢指针,慢指针指向当前处理的元素位置,快指针指向寻找新的不同元素的位置
   3. 本题特征:有序数组,所以相同元素必相邻
3. 没有关注到题目中的信息:**不需要考虑数组中超出新长度后面的元素**,其实后面的数组元素根本就保持,并没有被删除,只需要考虑前 k 个
4. 二次尝试，秒了

## 27 移除元素 + + ✔

1. 双指针,融会贯通![双指针](images/image-11.png)
2. 二次尝试秒了

## 35 搜索插入位置 + + ✔

1. flag 标记是否存在比 target 小的元素,时间复杂度不符合要求,为 `O(n)`;没有利用数组是有序的这一特征，46.99% 99.03%
2. 看到有序数组,又要求时间复杂度为 `O(logn)`，立马想到二分法!!70.41% 77.4%
   1. 注意 middle 是下标,取值和取下标分清楚
   2. 注意和 target 比较之后,区间别反了
   3. 相等的时候记得返回值
3. 二次尝试，想了一下，秒了

## 66 加一 + +

1. 简单情况简单处理,针对 9 特殊处理,但是发现数值太大![数值太大,出现NaN](images/image-15.png)
2. 尝试用 BigInt 处理,仍存在问题![BigInt仍无法解决](images/image-14.png),发现是 Math.pow()方法只能对 number 类型数据进行幂运算,而无法对 BigInt 运算,将 BigInt 类型转换成 Number 类型会报错 TypeError
3. 一些知识点巩固
   1. sort 是返回一个新数组
   2. reverse 是直接原地修改原数组
4. 找不出问题![代码](images/image-16.png)![测试](images/image-17.png)
5. 我的最终解法![代码](images/image-18.png)![性能](images/image-19.png)
   1. 对长度为 1 的单独判断
   2. 逆序判断 num 是否为 9，是 9 则置 0 并递归，不是 9 则 num+1 直接返回
6. top voted solution，直接在原数组上操作，每一位都是 9 的情况，在 for 循环结束的时候 return，同时要在数组前面添加一个 1，因为要补位![性能](images/image-20.png)
7. 二次尝试 unshift 方法知识点巩固
   1. unshift 方法是直接在原数组上面修改
   2. 所以需要先原地修改，再将 digits 返回（分两步）

## 88 合并两个有序数组 + + ✔

1. 思考的方式
2. 边界条件的考虑，总是不关注题目的数据范围：0 <= m, n <= 200
3. splice 不熟悉：返回被删除的元素,在原数组上修改
4. 考虑到了两个指针，考虑到了从后面开始赋值
5. 做出来了，效果很差，![运行时间和占空间都很差](images/image-21.png)
6. 直接合并后 sort![时间复杂度很好](images/image-22.png)
   - JavaScript 的内置方法 sort 方法，在大多数浏览器中，平均时间复杂度是 `O((m+n)log(m+n))`，完全不符合要求
   - 但是看起来效果很好
7. 双指针实现，while 循环，这个逻辑更精简![时间空间复杂度都较好](images/image-23.png)
8. 一些数组知识
   1. 想要在原数组上修改数据，不会实现
9. 终于做出来了，也是用双指针，达到了所要求的时间复杂度![比较两个数组中的元素并顺序合并+处理nums2中的剩余元素](images/image-27.png)
   - 正因为会处理 nums2 中的剩余元素，也就不需要单独考虑 m=0 了
10. 基于上面方法，进一步优化![再优化性能](images/image-40.png)
11. 二次尝试，一次过啦而且时间复杂度符合要求![Alt text](images/image-52.png)

## 118 杨辉三角 + + + !

1. 先初始化所有的 1，不然后面没办法加，两次双层循环
2. 记住相加逻辑`arr[i][j] = arr[i - 1][j] + arr[i - 1][j - 1]`
3. top voted solution 的小技巧
   1. 通过 `res.push(Array(i + 1))`初始化，在添加二维数组的基础上，限定了数组的元素个数
   2. 没有两次双层循环，而是 if-else，因为在计算到某个 arr[i][j]的时候，必然得到了`arr[i - 1][j]` 和 `arr[i - 1][j - 1]`，理解清楚这层关系；必然是从第一行开始创建~
4. 逻辑还很不熟练
5. top voted solution![Alt text](images/image-35.png)
   1. 就是双层 for 循环
   2. if-else 两种情况进行赋值，赋 1 的写在前面
6. 二次尝试，仍然会出现报错！
   1. 让数组变成二维数组的两种方法，我混用了
      1. 声明一个空数组，给数组新增元素（push 数组），使之变成二维数组
      2. 声明一个 numRows 个未定义元素的数组，给数组的元素设定为数组（赋值），使之成为二维数组
   2. 数组的大小一定要争取
   3. if 语句的判断一定要正确
7. 终于完成，但是出错多次![Alt text](images/image-53.png)
8. `let arr = Array(numRows).map(item => Array(numRows).fill(0))`没有起到我理解的作用
   1. 因为：**未初始化的数组（sparse array）不会执行 map 函数的回调**
   2. 想达到这个效果需要：`let arr = Array.from({ length: numRows }).map(() => Array(numRows).fill(0));`
   3. 数组元素的值为`empty`和`undefined`是不一样的哦~empty是未初始化的，undefined是初始化了的，map不会对稀疏数组进行回调
   4. 或者需要：`let arr = Array(numRows).fill(0).map(()=>new Uint32Array(numRows))`
9. 

## 119 杨辉三角 Ⅱ + + + 

1. 不会做
2. 只会用和上面同样的方法![时间复杂度不符合要求](images/image-29.png)
3. top voted solution![效果很好](images/image-36.png)
4. 二次尝试，一次过![Alt text](images/image-54.png)
5. 杨辉三角某行某列元素的计算公式，从同行的前一个元素推理
   1. 第r行（r从0-n），第j列元素
   2. （非列首和列尾元素）a[r][j] = a[r][j-1]*(r-j+1)/j
   3. 如第5行4列的元素a[5][4] = a[5][3]*2/4 = 10 * 2 / 4 正确

## 121 买股票的最佳时机 + + + !!

1. Math.max()方法要将一个个数传进去，所以数组要用扩展运算符
2. 我的方法一，将每一个元素与后面的所有元素组成的数组进行比较，得出一个最大值，但是超出时间限制![超出时间限制](images/image-28.png)
3. 我的方法二，将数组的数值和索引存入对象，并对数组进行排序，好像难以实现？
4. 我的方法三，直接找出元素的最大值，如果它的索引不为 0，就找出下标比他小的数组最小值；[3,3,5,0,0,3,1,4]这个例子推翻了我的方法。![存在问题](images/image-30.png)
5. 用新方法，数组解构，递归，时间超出限制![Alt text](images/image-37.png)
6. 还是没掌握用栈？等数据结构代替递归
7. 自己思路不清，双指针秒了；之前考虑过双指针，但是没做出来![Alt text](images/image-38.png)
8. 二次尝试，问题
   1. while 循环退出条件没理清楚，循环里面更改了控制循环条件的变量，导致超时
   2. **细节问题：将左边更大作为一类，另外两种作为另一类，左边更大的时候，只移动左指针，否则移动右指针**
      1. 等于的时候，完全不可能成为current值，相当于直接让right右移，但是等于的时候不需要将left右移
   3. 不需要考虑剩余部分
      1. left并不会移到数组长度-1的下标位置
9. 还是没理解这种解法，之前秒了，现在写不出了
10. 贪心算法，时间复杂度O(n)，空间复杂度O(1)![121贪心算法](image-3.png)
    1.  贪心算法思想：维护一个最小值
    2.  lowerPrice：维护已经遍历过的数据中的最小值
    3.  profit：维护当前遍历过的最大利润
11. 动态规划思想，这题，时空效率都很低
    1.  ![121动态规划算法](image-4.png)
12. tips：通过调用数组实例身上的fill方法，来初始化数组
13. 再次尝试，数组的初始化是个大问题，行和列别反了！

## 169 多数元素 + + +!✔

1. map 设置元素值总要用 set 方法
2. 完成了，性能不错![思路清晰一遍过](images/image-31.png)
3. 二次尝试，问题很多，细节把握总是要通过结果来检验，自己不能够找出错误![Alt text](images/image-55.png)
   1. 至少有四个细节
   2. 我的方法是先将数组排序，用两个指针（可以减少为一个指针）以及count记录当前数量以及目标的数量
4. 所谓set方法，根本不符合题目要求的空间复杂度为O(1)的要求！毫无参考价值
5. top voted solution（还不理解）![Alt text](images/image-56/png)
   1. 候选主要元素的含义，总是保存已经遍历过的元素中数量最多的那个（相等的话保存前面出现的那个）
   2. 计数器的含义
6. 算法步骤如下：
   1. 初始化一个候选主要元素 sol 和一个计数器 cnt。
   2. 对于数组中的每一个元素：
      1. 如果计数器为 0，我们把当前元素设为候选主要元素，并把计数器设为 1。
      2. 如果当前元素与候选主要元素相同，我们增加计数器。
      3. 否则，我们减少计数器。
   3. 由于主要元素超过 n/2，所以最终，候选主要元素 sol 就是数组中的主要元素。
7. 来理解这个算法的关键：
   1. 当我们遇到与当前候选主要元素不同的元素时，我们减少计数器。
   2. 当计数器为 0 时，我们更改候选主要元素。
   3. 由于主要元素的数量超过 n/2，我们可以保证在算法结束时，sol 是主要元素。
8. 注意：假设主要元素总是存在的
9. 

## 217 存在重复元素 + + ✔

1. 和 169 实现方法一样，性能很差，主要是空间复杂度过高![Map空间复杂度高](images/image-32.png)
2. 使用 Object，多次提交终于做对，性能也很差，和 Map 差的不行上下![Alt text](images/image-33.png)
3. 用两个指针的逻辑，没想清楚，没做出来
4. 用 reduce 方法，为什么 prev 会是 undefined？![reduce方法使用](images/image-34.png)
5. 第二天脑子清醒，sort+双指针，秒了；但是时间复杂度不是很好 26.68 % 88.61 % 
6. 用set去重，用size获取set里面元素个数 61.29 % 34.61 %

## 219 存在重复元素 II + ！

1. 暴力法，改良版
2. 用哈希表来记录每个元素最近一次出现的位置
   1. key保存元素值
   2. value保存最近一次出现的索引值

## 34.在排序数组中查找元素的第一个和最后一个位置 + !

1. 用 JavaScript 直接秒了，没意义!因为时间复杂度不符合要求，无论是 indexOf 还是 lastIndexOf，时间复杂度都是 `O(n)`[indexOf 和 lastIndexOf 方法](images/image-41.png)
2. top voted solution 用双指针的思路![双指针](images/image-42.png)，没理解
3. 看完题解自己尝试，结果很好。![双指针效果很好](images/image-43.png)
4. 题解注意事项
   1. 写一个箭头函数，用双指针
   2. **`>=`两种情况，统一处理方式**，为了保证找到的是第一个和 target 相等的元素，如果直接找到相等的返回，会导致找到后面的 target
   3. 边界条件判断，找到的索引对应的元素之不相等，表示没找到，就算是没找到，这个得到的索引对应的元素也是比 target 小但是最接近 target 的元素
   4. 理解第二次调用的时候用 target+1 传入的含义
5. 这个题目的解题精髓就在于target+1
6. 看到有序数组+时间复杂度要求O(logn) 就立马想到二分法
7. 

## 69 x-的平方根 + + ！

1. 不会用二分法的思想
2. 直接求平方，时间效率低![直接求平方](images/image-44.png)
3. 模仿 top voted solution 的二分法思想![二分法](images/image-45.png)
4. 看清楚边界条件x=0
5. **很多地方都要用到二分法，但是各处的返回下标各不相同！！！！！**
6. 理解题目:**题目要求求解x的平方根，并且需要返回整数部分。如果结果不是一个整数，我们需要返回小于x的平方根的最大整数。**
7. 注意二分查找法的特点：当我们把查找范围缩小一半时，left总是指向查找范围的开始，而right总是指向查找范围的结束。
8. 二分法结束的时候，一定会是一个状态，就是left>right。left是第一个使得其平方大于x的整数，而right恰好是最后一个使得其平方小于或等于x的整数。最后的left和right一定是相邻的两个索引值。

## 367 有效的完全平方数 + + ✔

1. 和 69 题几乎一样，秒了![Alt text](images/image-46.png)
2. 边界条件考虑!
3. 这个题目完全应该将right值设置为(num >> 1) + 1,才能降低时空复杂度!!

# 977 有序数组的平方 + + !

1. map 方法是生成新的数组，不是在原数组身上修改，对于数组的方法，还是不熟练，时间复杂度一般![map+sort方法](images/image-47.png)
2. map 方法和 sort 方法的时间复杂度？
3. top voted solution 时间复杂度![Alt text](images/image-48.png)
4. 为什么这个题目也可以用双指针？因为：利用了 nums 数组有序的特点，只需要比较两端的平方值，即可得出当前的最大平方值；存在正负数，绝对值越大的，平方值越大。
5. 对空间复杂度没有要求，用一个空数组来存放数据
6. 尝试用双指针的思想，一些细节问题
   1. 要复制的是平方后的值
   2. left 是++，而不是--
7. 感觉做题目没动脑子,没有去考虑正负数的平方的关系,完全可以用双指针解决问题

# 209 长度最小的子数组 + + !!

1. 题目看的不仔细，做错很多次
2. 在修改代码的过程中，细节没注意，犯低级错误，警醒
3. 用双指针做出来了，时间空间复杂度都很差![双指针](images/image-49.png)，我这个解法的时间复杂度是`O(n^2)`还是`O(n)`！！！应该只是两次遍历数组，2n 的时间复杂度，因此是`O(n^2)`？s
4. 优化
   1. 首先只需要求最小的元素个数，就只需要一个变量来存储，每次得到就进行比较，可以大大降低空间复杂度
   2. 滑动窗口，时间复杂度为![滑动窗口](images/image-51.png)
   3. 注意，实际`console.log`语句会消耗事件，不要让无关的影响效率！
5. 果然，我的双指针，效果比其他的效果都好！![我的双指针法](images/image-50.png)
   1. 其实也是滑动窗口？
6. 亲,这并不适合用动态规划的方法,时空效率贼低16.35 % 5.01 %
7. 题目要求O(n)时间复杂度,进阶要求O(logn)时间复杂度
8. 滑动窗口,将left和right的值都初始化为0,从而逻辑可以简化,不必考虑那么多的边界条件,但是时间复杂度是O(n)
9. 

# 1289 下降路径最小和II

1. 动态规划
2. 回溯法
3. 记忆化搜索
4. 堆（优先队列）

# 509 斐波那契数 + 

1. 递归
2. 动态规划

# 746 使用最小花费爬楼梯 +

1. 动态规划
2. 动态规划+直接在原数组上操作
3. 累加的次数，由步数决定

# 377 组合总和-ⅳ 

1. 动态规划
2. dp的长度？目标值+1
3. 初始化
4. 累加的次数，由数组里面符合范围的值的个数决定

# 62 不同路径

1. 动态规划（更推荐）
2. 组合问题，递归
   1. 阶乘计算公式`C(a, b) = a!/(b!*(a-b)!)``
   2. C(m+n-2, m-1) 或 C(m+n-2, n-1)

# 63 不同路径-ii

1. Uint32Array会自动初始化值为0
2. i=0和j=0有多处需要特殊处理

# 343 整数拆分

1. 动态规划
   1. 没有理解这里的dp不是要求累加值，是要求最大值
   2. 初始值总是从第一个1开始设置！
   3. 数字i拆分乘积和数字i本身的大小不确定（大部分确定，前几个自然数不确定）
   4. num的取值（nums数组中的是数字）直接通过i来控制循环！
2. 贪心算法


